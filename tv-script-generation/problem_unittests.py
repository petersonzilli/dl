...
def test_build_nn(build_nn):
    with tf.Graph().as_default():
        ...
        assert logits.get_shape().as_list() == test_input_data_shape + [test_vocab_size], \
            'Outputs has wrong shape. Expected shape {}. Found shape {}'.format(test_input_data_shape + [test_vocab_size], logits.get_shape())
        assert final_state.get_shape().as_list() == [test_rnn_layer_size, 2, None, test_rnn_size], \
            'Final state wrong shape. Expected shape {}. Found shape {}'.format([test_rnn_layer_size, 2, None, test_rnn_size], final_state.get_shape())
